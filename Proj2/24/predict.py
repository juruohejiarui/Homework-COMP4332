...
def run_one_dataset(dataset_dir: Path, task: str, out_dir: Path, max_context: int, seed: int, device: str, scale: bool):
    ...
    pred_path = out_dir / "predict-ctx.csv"
...
def main():
    ...
    ap.add_argument("--out_root", type=str, default="data", help="输出目录")
    ap.add_argument("--max_context", type=int, default=40000, help="上下文最大训练行数（太大可能超上下文限制/变慢）")
    ap.add_argument("--scale", action="store_true", help="是否对数值特征进行标准化")
    ap.add_argument("--seed", type=int, default=None)
    ap.add_argument("--device", type=str, default="cuda", help="cpu 或 cuda")
    args = ap.parse_args()
